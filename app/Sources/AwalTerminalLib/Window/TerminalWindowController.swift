...
class TerminalWindowController: NSWindowController, NSWindowDelegate, CustomTabBarDelegate {
    ...
    var activeDiffReview: DiffReviewController? {
        diffReviewPopover?.contentViewController as? DiffReviewController
    }
    ...
    private func wireACPCallbacks(_ client: ACPClient, tab: TabState) {
        ...
        client.onFsWriteRequest = { [weak self] requestId, path, content in
            ...
            self.activeTab.agentChanges.recordWrite(requestId: requestId, path: path, content: content, cwd: cwd)
            self.updateAgentChangesTree()

            // Show diff review popover instead of plain permission dialog
