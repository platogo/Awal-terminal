...
class PipelineDAGView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for sub in subagents {
            let depth = resolveDepth(sub.id, depths: &depths)
            depths[sub.id] = depth
        }
    ...
    private func resolveDepth(_ id: String, depths: inout [String: Int]) -> Int {
        if let d = depths[id] { return d }
        guard let sub = subagents.first(where: { $0.id == id }) else { return 0 }
        if sub.dependsOn.isEmpty { return 0 }
        let maxParent = sub.dependsOn.map { resolveDepth($0, depths: &depths) }.max() ?? 0
        return maxParent + 1
    }
    ...
    override var intrinsicContentSize: NSSize {
        ...
        let vSpacing: CGFloat = 16
        var depths: [String: Int] = [:]
        for sub in subagents { depths[sub.id] = resolveDepth(sub.id, depths: &depths) }
        let maxDepth = depths.values.max() ?? 0
        var columnCounts = [Int](repeating: 0, count: maxDepth + 1)
